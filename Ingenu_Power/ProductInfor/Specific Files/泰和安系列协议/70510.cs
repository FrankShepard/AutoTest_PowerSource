...
{
	/// <summary>
	/// 继承自Base的 IG-M3202F 电源的相关信息
	/// </summary>
	public class _70510 : Base
	{
		#region -- 产品通讯相关属性
		...
		public enum UserCmd : byte
		{
			...
			/// 查询电源工作状态 - 巡检
			/// </summary>
			UserCmd_QueryWorkingStatus = 0x80,
			/// <summary>
			/// 复位 - 通讯复位，消音状态恢复默认
			/// </summary>
			UserCmd_Reset = 0x81,
			...
			/// 自检
			/// </summary>
			UserCmd_SelfCheck = 0x83,
			/// <summary>
			/// 查询产品类型
			/// </summary>
			UserCmd_QueryProductType = 0x92,
			
			/// <summary>
			/// 查询备电输入的电压和电流
			/// </summary>
			UserCmd_QuerySp = 0xC0,
			/// <summary>
			/// 查询备电欠压点对应的电压值
			/// </summary>
			UserCmd_QuerySpUnderVoltage = 0xC1,
			/// <summary>
			/// 查询备电切断点对应的备电电压
			/// </summary>
			UserCmd_QuerySpCutoffVoltage = 0xC2,
			/// <summary>
			/// 查询“充电完成”标志对应的备电电压值
			/// </summary>
			UserCmd_QueryChargeCompletedVoltage = 0xC3,
			/// <summary>
			/// 查询备电切断之后蜂鸣器工作的时间
			/// </summary>
			UserCmd_QueryBeepWorkingTime = 0xC4,
			/// <summary>
			/// 查询输出通道的电压和电流
			/// </summary>
			UserCmd_QueryOutput = 0xC5,			
			/// <summary>
			/// 查询“输出开路”标志对应的电流值
			/// </summary>
			UserCmd_QueryOutputOpenCurrent = 0xC6,
			/// <summary>
			/// 查询“输出过功率”对应功率值 / OCP值；包含了总输出功率和指定单通道的功率值
			/// </summary>
			UserCmd_QueryOverpower = 0xC7,

			/// <summary>
			/// 设置电源的地址
			/// </summary>
			UserCmd_SetAddress = 0xD1,
			/// <summary>
			/// 设置电源产品通讯波特率
			/// </summary>
			UserCmd_SetBaudrate = 0xD2,
			/// <summary>
			/// 设置主电工作与否
			/// </summary>
			UserCmd_SetMpWorkEnable = 0xD3,
			/// <summary>
			/// 设置是否允许备电工作
			/// </summary>
			UserCmd_SetSpWorkEnable = 0xD4,
			/// <summary>
			/// 设置是否允许对备电进行充电
			/// </summary>
			UserCmd_SetChargeEnable = 0xD5,
			/// <summary>
			/// 设置备电欠压点对应的电压值
			/// </summary>
			UserCmd_SetSpUnderVoltage = 0xD6,
			/// <summary>
			/// 设置备电切断点对应的备电电压
			/// </summary>
			UserCmd_SetSpCutoffVoltage = 0xD7,
			/// <summary>
			/// 设置“充电完成”对应的备电电压
			/// </summary>
			UserCmd_SetChargeCompletedVoltage = 0xD8,
			/// <summary>
			/// 设置备电关断之前的维持时间
			/// </summary>
			UserCmd_SetBeepWorkingTime = 0xD9,
			/// <summary>
			/// 设置是否允许输出
			/// </summary>
			UserCmd_SetOutputEnable = 0xDA,
			/// <summary>
			/// 设置“输出开路”对应的电流值
			/// </summary>
			UserCmd_SetOutputOpenCurrent = 0xDB,
			/// <summary>
			/// 设置“输出过载”对应的功率值 - 包含对应通道的过载值和输出总过载值
			/// </summary>
			UserCmd_SetOutputOverpower = 0xDC,
			/// <summary>
			/// 进入校准模式
			/// </summary>
			UserCmd_GetInValidationMode = 0xDF,
			/// <summary>
			/// 操作命令无效（操作指令错误或者指令码校验不正确）
			/// </summary>
			UserCmd_RespondError = 0xFF			
		}
		...
		public override void Communicate_User( SerialPort serialPort, out string error_information )
		{
			...
			int index = 0;
			/*判断串口打开是否正常，若不正常则先要进行打开设置*/
			try { if ( !serialPort.IsOpen ) { serialPort.Open ( ); } } catch(Exception ex) {
				error_information = ex.ToString( );
				error_information += "待测产品 出现了不能通讯的情况（无法打开串口），请注意此状态"; return; }
			...
			for (int temp_index = 0; temp_index < 5; temp_index++) {
				...
				if ( temp_index == 0 ) {
					...
					sent_data = Product_vCmdQuery( UserCmd.UserCmd_QueryOutput , 2 , out error_information );
				} else if ( temp_index == 3 ) {
					sent_data = Product_vCmdQuery( UserCmd.UserCmd_QueryOutput , 3 , out error_information );
				} else if ( temp_index == 4 ) {
					sent_data = Product_vCmdQuery( UserCmd.UserCmd_QuerySp , 0 , out error_information );
				}
				if (error_information != string.Empty) { return; }
		...
		/// <param name="error_information">可能存在的异常信息</param>
		/// </summary>
		private void Communicate_User_SetBeepTime(SerialPort serialPort, out string error_information)
		{
			error_information = string.Empty;
			int index = 0;
			...
			byte[] sent_data = Product_vCmdSet_BeepWorkingTime( 4200 );
			if (error_information != string.Empty) { return; }
		...
		{
			byte[] SerialportData = new byte[] { 0, 0, 0, 0, 0, 0 };
			error_information = string.Empty;
			if(!((userCmd == UserCmd.UserCmd_QueryWorkingStatus) || (userCmd == UserCmd.UserCmd_QueryProductType) 
				|| (userCmd >= UserCmd.UserCmd_QuerySp ) && (userCmd <= UserCmd.UserCmd_QueryOverpower))) {
				error_information = "传递查询命令出现范围错误";
				return SerialportData;
			}
			
			SerialportData[ 0 ] = 230; //默认产品地址为230
			
			if((userCmd >= UserCmd.UserCmd_QueryOutput) && (userCmd <= UserCmd.UserCmd_QueryOverpower )) {
				SerialportData[ 1 ] = 4; //包含特定通道的信息，长度为4
				SerialportData[ 2 ] = ( byte ) userCmd;
				SerialportData[ 3 ] = ( byte ) channel_index;
				byte[] tmp = BitConverter.GetBytes( Product_vGetCalibrateCode( SerialportData, 0, SerialportData[ 1 ] ) );
				SerialportData[ 4 ] = tmp[ 0 ];
				SerialportData[ 5 ] = tmp[ 1 ];
			} else {
				SerialportData[ 1 ] = 3; //默认长度为3
				SerialportData[ 2 ] = ( byte ) userCmd;
				byte[] tmp = BitConverter.GetBytes( Product_vGetCalibrateCode( SerialportData, 0, SerialportData[ 1 ] ) );
				SerialportData[ 3 ] = tmp[ 0 ];
				SerialportData[ 4 ] = tmp[ 1 ];
				
			}
			byte[] send_data = new byte[ SerialportData[ 1 ] + 2 ];
			Buffer.BlockCopy( SerialportData, 0, send_data, 0, ( SerialportData[ 1 ] + 2 ) );
			return send_data;
		}
		...
		private byte[] Product_vCmdSet_OutputOpenCurrent(int channel_index,decimal target_current)
		{
			...
			SerialportData[ 0 ] = 230; //默认地址为230
			SerialportData[ 1 ] = 6; 
			SerialportData[ 2 ] = ( byte )UserCmd.UserCmd_SetOutputOpenCurrent;
			SerialportData[ 3 ] = ( byte ) channel_index;
			UInt16 target = Convert.ToUInt16( target_current * 1000 ); //单位统一为mA
			byte[] temp = BitConverter.GetBytes( target );
			SerialportData[ 4 ] = temp[ 0 ];
			SerialportData[ 5 ] = temp[ 1 ];			
			temp = BitConverter.GetBytes( Product_vGetCalibrateCode( SerialportData, 0, SerialportData[ 1 ] ) );
		...
		/// <summary>
		/// 输出通道(从1开始)的OXP设置；总功率/电流设置时通道数为 0  此处为OCP
		/// </summary>
		/// <param name="channel_index">输出通道</param>
		/// <param name="target_current">目标电流</param>
		/// <returns>具体的命令字节</returns>
		private byte[] Product_vCmdSet_OutputOverCurrent(int channel_index, decimal target_current)
		{
			byte[] SerialportData = new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 };
			SerialportData[ 0 ] = 230; //默认地址为230
			SerialportData[ 1 ] = 6;
		...
		private byte[] Product_vCmdSet_SpChargeCompletedVoltage(decimal target_voltage)
		{
			...
			UInt16 target = Convert.ToUInt16( target_voltage * 1000 ); //单位统一为mV
			byte[] temp = BitConverter.GetBytes( target );
			SerialportData[ 3 ] = temp[ 0 ];
			SerialportData[ 4 ] = temp[ 1 ];
			temp = BitConverter.GetBytes( Product_vGetCalibrateCode( SerialportData, 0, SerialportData[ 1 ] ) );
			SerialportData[ 5 ] = temp[ 0 ];
			SerialportData[ 6 ] = temp[ 1 ];
			return SerialportData;
		}
		...
		/// <param name="target_voltage">目标电压</param>
		/// <returns>具体的命令字节</returns>
		private byte[] Product_vCmdSet_SpUnderVoltage(decimal target_voltage)
		{
			byte[] SerialportData = new byte[] { 0, 0, 0, 0, 0, 0, 0 };
			SerialportData[ 0 ] = 230; //默认地址为230
			SerialportData[ 1 ] = 5;
		...
		private byte[] Product_vCmdSet_Enable(UserCmd userCmd ,int channel_index, bool working_status,out string error_information)
		{
			...
			if(!((userCmd == UserCmd.UserCmd_SetMpWorkEnable) || (userCmd == UserCmd.UserCmd_SetSpWorkEnable) 
				|| (userCmd == UserCmd.UserCmd_SetChargeEnable) || (userCmd == UserCmd.UserCmd_SetOutputEnable))) {
			...
			if(working_status != false) {
				SerialportData[ 3 ] = 0x01;
			}
			byte[] temp = BitConverter.GetBytes( Product_vGetCalibrateCode( SerialportData, 0, SerialportData[ 1 ] ) );
			SerialportData[ 4 ] = temp[ 0 ];
			SerialportData[ 5 ] = temp[ 1 ];
			return SerialportData;
		}
		...
		/// </summary>
		/// <returns>具体的命令字节</returns>
		private byte[] Product_vSelfCheck()
		{
			byte[] SerialportData = new byte[] { 0, 0, 0, 0, 0 };
			SerialportData[ 1 ] = 3;
			SerialportData[ 2 ] = ( byte ) UserCmd.UserCmd_SelfCheck;
			byte[] temp = BitConverter.GetBytes( Product_vGetCalibrateCode( SerialportData, 0, SerialportData[ 1 ] ) );
			SerialportData[ 3 ] = temp[ 0 ];
			SerialportData[ 4 ] = temp[ 1 ];
			return SerialportData;
		...
		public override string Product_vCheckRespond( byte[] sent_cmd, SerialPort sp_product, out byte [ ] received_cmd )
		{
			...
			try {
				if (sp_product.BytesToRead > 0) {
					...
					do {
						if (received_data[ real_data_startindex ] == 0x01) {
							break;
						}
					...
					if (received_cmd.Length >= 5) {
						UInt16 validatecode = BitConverter.ToUInt16( received_cmd, received_cmd.Length - 2 );
						if (validatecode != Product_vGetCalibrateCode( received_cmd, 0, received_cmd.Length - 2 )) {
		...
		public override string Product_vGetQueryedValue(byte[] sent_data, byte [ ] SerialportData )
		{
			...
			try {
				//提取需要查询的有效数据
				switch(( UserCmd )SerialportData[1]) {
					case UserCmd.UserCmd_QuerySp:						
						infor_Uart.Measured_SpVoltage[0] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						break;
					case UserCmd.UserCmd_QueryOutput:
						infor_Uart.Measured_OutputVoltage[ sent_data[3] - 1 ] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						infor_Uart.Measured_OutputCurrent[ sent_data[3] - 1 ] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 4 ) ) / 1000m;
						break;
					case UserCmd.UserCmd_QueryWorkingStatus:
						infor_Uart.communicate_Signal.communicate_Signal_Mp.Measured_MpErrorSignal = Convert.ToBoolean( SerialportData[ 3 ] & 0x01 );
						infor_Uart.communicate_Signal.communicate_Signal_Mp.Measured_MpLostSignal = Convert.ToBoolean( SerialportData[ 4 ] & 0x01 );
						infor_Uart.communicate_Signal.communicate_Signal_Mp.Measured_MpUndervoltageSignal = Convert.ToBoolean( SerialportData[ 4 ] & 0x02 );
						infor_Uart.communicate_Signal.communicate_Signal_Mp.Measured_MpOvervoltageSignal = Convert.ToBoolean( SerialportData[ 4 ] & 0x04 );
						
						infor_Uart.communicate_Signal.communicate_Signal_Sp.Measured_SpErrorSignal = Convert.ToBoolean( SerialportData[ 3 ] & 0x02 );
						infor_Uart.communicate_Signal.communicate_Signal_Sp.Measured_SpShortSignal = Convert.ToBoolean( SerialportData[ 5 ] & 0x01 );
						infor_Uart.communicate_Signal.communicate_Signal_Sp.Measured_SpOpenSignal = Convert.ToBoolean( SerialportData[ 5 ] & 0x02 );
						infor_Uart.communicate_Signal.communicate_Signal_Sp.Measured_SpConnectErrorSignal = Convert.ToBoolean( SerialportData[ 5 ] & 0x04 );
						infor_Uart.communicate_Signal.communicate_Signal_Sp.Measured_SpUndervoltageSignal = Convert.ToBoolean( SerialportData[ 5 ] & 0x08 );
						infor_Uart.communicate_Signal.communicate_Signal_Sp.Measured_SpCutoffSignal = Convert.ToBoolean( SerialportData[ 5 ] & 0x10 );
						infor_Uart.communicate_Signal.communicate_Signal_Sp.Measured_SpBatsResistanceErrorSignal = Convert.ToBoolean( SerialportData[ 5 ] & 0x20 );

						infor_Uart.communicate_Signal.communicate_Signal_Output.Measured_OverpowerSignal = Convert.ToBoolean( SerialportData[ 3 ] & 0x08 );

						infor_Uart.communicate_Signal.communicate_Signal_Output.Measured_OutputErrorSignal = Convert.ToBoolean( SerialportData[ 3 ] & 0x04 );
						for (int channel_index = 0; channel_index < infor_Output.OutputChannelCount; channel_index++) {
							infor_Uart.communicate_Signal.communicate_Signal_Output.Measured_OutputOpenSignal[ channel_index ] = Convert.ToBoolean( SerialportData[ 6 ] & ( 0x01 << channel_index ) );
							infor_Uart.communicate_Signal.communicate_Signal_Output.Measured_OutputOverpowerSignal[ channel_index ] = Convert.ToBoolean( SerialportData[ 6 ] & ( 0x10 << channel_index ) );
							infor_Uart.communicate_Signal.communicate_Signal_Output.Measured_OutputInProtectingStatusSignal[ channel_index ] = Convert.ToBoolean( SerialportData[ 7 ] & ( 0x01 << channel_index ) );
							infor_Uart.communicate_Signal.communicate_Signal_Output.Measured_OutputUserCloseSignal[ channel_index ] = Convert.ToBoolean( SerialportData[ 7 ] & ( 0x10 << channel_index ) );
						}
						infor_Uart.communicate_Signal.communicate_Signal_Charge.Measured_IsChargingSignal = Convert.ToBoolean( SerialportData[ 8 ] & 0x01 );
						infor_Uart.communicate_Signal.communicate_Signal_Charge.Measured_ChargeCompletedSignal = Convert.ToBoolean( SerialportData[ 8 ] & 0x02 );
						infor_Uart.communicate_Signal.communicate_Signal_Charge.Measured_ChargeHardwareErrorSignal = Convert.ToBoolean( SerialportData[ 8 ] & 0x04 );

						infor_Uart.communicate_Signal.communicate_Signal_Other.Measured_UserStopMpSignal = Convert.ToBoolean( SerialportData[ 9 ] & 0x01 );
						infor_Uart.communicate_Signal.communicate_Signal_Other.Measured_UserStopChargingSignal = Convert.ToBoolean( SerialportData[ 9 ] & 0x02 );
						infor_Uart.communicate_Signal.communicate_Signal_Other.Measured_OutputFuseErrorSignal = Convert.ToBoolean( SerialportData[ 9 ] & 0x04 );
						break;
					case UserCmd.UserCmd_QueryOutputOpenCurrent:
						infor_Uart.Measured_OutputOpenMaxCurrent[ sent_data[ 3 ] - 1 ] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						break;
					case UserCmd.UserCmd_QueryOverpower:
						if (sent_data[ 3 ] > 0) {
							infor_Uart.Measured_OutputOverpowerValue[ sent_data[ 3 ] - 1 ] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						} else {
							infor_Uart.Measured_AllOutputOverpowerValue = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						}
						break;
					case UserCmd.UserCmd_QueryChargeCompletedVoltage:
						infor_Uart.Measured_ChargeCompletedVoltage[ 0 ] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						break;
					case UserCmd.UserCmd_QuerySpCutoffVoltage:
						infor_Uart.Measured_SpCutoffVoltage[ 0 ] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						break;
					case UserCmd.UserCmd_QuerySpUnderVoltage:
						infor_Uart.Measured_SpUnderVoltage[ 0 ] = Convert.ToDecimal( BitConverter.ToInt16( SerialportData, 2 ) ) / 1000m;
						break;
					case UserCmd.UserCmd_QueryBeepWorkingTime:
						infor_Uart.Measured_BeepWorkingTime = BitConverter.ToInt16( SerialportData, 2 );
						break;
					default:
						break;
				}
		...
		/// <summary>
		/// 计算校验值 - CRC16 modbus 的校验形式
		/// </summary>
		/// <param name="command_bytes">通讯使用的数组</param>
		/// <param name="start_index">数组中的起始字节索引</param>
		/// <param name="count">需要计算的字节长度</param>
		/// <returns>所需校验值</returns>
		private UInt16 Product_vGetCalibrateCode(byte[] command_bytes, Int32 start_index, Int32 count)
		{
			UInt16 tmp = 0, crc16 = 0xFFFF;
			for (int index = start_index; index < start_index + count; index++) {
				crc16 ^= command_bytes[ index ];
				for (int i = 0; i < 8; i++) {
					tmp = Convert.ToUInt16( crc16 & 0x0001 );
					crc16 >>= 1;
					if (tmp == 1) {
						crc16 ^= 0xA001; //异或多项式
					}
				...
				if ( temp_index == 0 ) {
					using ( MeasureDetails measureDetails = new MeasureDetails ( ) ) {
						using ( SerialPort serialPort = new SerialPort ( port_name, default_baudrate, Parity.None, 8, StopBits.One ) ) {
							...
							//用户模式时设置蜂鸣器时间
							Communicate_User_SetBeepTime( serialPort, out error_information );
							if(error_information != string.Empty) { continue; }
							...
							if ( whole_function_enable == false ) { //上下限检测即可
								...
								for ( index = 0 ; index < 2 ; index++ ) {
									...
									if ( generalData_DCPower.ActrulyCurrent < 0.06m ) {
										break;
									}
								}
					...
					using ( MeasureDetails measureDetails = new MeasureDetails ( ) ) {
						using ( SerialPort serialPort = new SerialPort ( port_name, default_baudrate, Parity.None, 8, StopBits.One ) ) {
							...
							for ( int index_of_channel = 0 ; index_of_channel < infor_Output.OutputChannelCount ; index_of_channel++ ) {
								...
								if ( Math.Abs( infor_Uart.Measured_OutputVoltage[ index_of_channel ] - real_voltage ) > 0.5m ) {
									error_information = "电源测试得到的输出电压" + (index_of_channel + 1).ToString() + "超过了合格误差范围 " + infor_Uart.Measured_OutputVoltage[ index_of_channel ].ToString( ) + "  " + real_voltage.ToString( );
								}
								if ( Math.Abs( infor_Uart.Measured_OutputCurrent[ index_of_channel ] - real_current ) > 0.5m ) {
									error_information = "电源测试得到的输出电流" + ( index_of_channel + 1 ).ToString() + "超过了合格误差范围 " + infor_Uart.Measured_OutputCurrent[ index_of_channel ].ToString( ) + "  " + real_current.ToString( );
								}
								if(error_information != string.Empty) { break; }
							}
							if (error_information != string.Empty) { continue; }
					...
					using (MeasureDetails measureDetails = new MeasureDetails()) {
						using (Itech itech = new Itech()) {
							using (SerialPort serialPort = new SerialPort( port_name, default_baudrate, Parity.None, 8, StopBits.One )) {
								...
								//根据满载电流值进行欠压状态的等待延时
								if (infor_Output.FullLoadType == LoadType.LoadType_CC) {
									Thread.Sleep( Convert.ToInt32( 50 * infor_Output.FullLoadValue[ 0 ] ) );
								}
								...
								for ( int index = 0 ; index < infor_Output.OutputChannelCount ; index++ ) {
									if ( infor_Output.Stabilivolt [ index ] == false ) {
										...
										if (whole_function_enable) {
											...
											if (value < infor_Output.Qualified_OutputVoltageWithLoad[ index, 0 ] * 0.1m) { //说明被捕获
									...
									for ( int index_of_load = 0 ; index_of_load < MeasureDetails.Address_Load_Output.Length ; index_of_load++ ) {
										if ( allocate_channel [ index_of_load ] == index_of_channel ) {
											...
											if ( generalData_Load.ActrulyVoltage < 0.95m * infor_Output.Qualified_OutputVoltageWithLoad [ index_of_channel, 0 ] ) {
												check_okey = false;
												error_information += "主电欠压输出通道 " + ( index_of_channel + 1 ).ToString ( ) + " 存在跌落   " + generalData_Load.ActrulyVoltage.ToString();
												continue;
											}
											break;
										}
